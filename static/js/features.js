...
function openPollModal() {
    ...
    if (pollOptions) {
        pollOptions.innerHTML =
            '<div class="poll-option-input"><input type="text" placeholder="' + _t('poll.option', '옵션') + ' 1" maxlength="100"></div>' +
            '<div class="poll-option-input"><input type="text" placeholder="' + _t('poll.option', '옵션') + ' 2" maxlength="100"></div>';
    }
...
async function createPoll() {
    ...
    if (!question) {
        showToast(_t('poll.question_required', '질문을 입력해주세요.'), 'warning');
        return;
    }
    if (options.length < 2) {
        showToast(_t('poll.options_required', '최소 2개의 옵션이 필요합니다.'), 'warning');
        return;
    }
    ...
    if (uniqueOptions.length !== options.length) {
        showToast(_t('poll.options_duplicated', '중복된 옵션이 있습니다.'), 'warning');
        return;
    }
    ...
    try {
        ...
        if (result.success) {
            showToast(_t('feature.poll.create', '투표가 생성되었습니다.'), 'success');
        ...
        showToast(_t('feature.poll.create_failed', '투표 생성에 실패했습니다.'), 'error');
    }
}
...
async function votePoll(pollId, optionId) {
    try {
        ...
        showToast(_t('poll.vote_failed', '투표에 실패했습니다.'), 'error');
    }
}
...
async function closePoll(pollId) {
    if (!currentRoom) return;
    if (!confirm(_t('feature.poll.close_confirm', '이 투표를 종료하시겠습니까?'))) return;

    try {
        var result = await api('/api/polls/' + pollId + '/close', { method: 'POST' });
        if (result.success) {
            showToast(_t('poll.closed', '투표가 종료되었습니다.'), 'success');
            loadRoomPolls();
        } else {
            showToast(_localizedError(result, _t('poll.close_failed', '투표 종료 실패')), 'error');
        }
    } catch (e) {
        showToast(_t('poll.close_failed', '투표 종료에 실패했습니다.'), 'error');
    }
}
...
async function loadRoomPolls() {
    ...
    try {
        ...
        if (typeof showToast === 'function') {
            showToast(_t('poll.load_failed', '투표 목록을 불러올 수 없습니다.'), 'error');
        }
    }
}
...
function createPollElement(poll) {
    ...
    var statusBadge = isClosed
        ? '<span class="poll-status closed">' + _t('poll.status.closed', '종료됨') + '</span>'
        : '<span class="poll-status active">' + _t('poll.status.active', '진행중') + '</span>';
    ...
    if (poll.ends_at && !isClosed) {
        ...
        if (diffMs > 0) {
            ...
            var timeText;
            if (diffDays > 0) {
                timeText = _t('poll.deadline_days', '{count}일 후 마감', { count: diffDays });
            } else if (diffHours > 0) {
                timeText = _t('poll.deadline_hours', '{count}시간 후 마감', { count: diffHours });
            } else {
                var diffMinutes = Math.floor(diffMs / (1000 * 60));
                timeText = _t('poll.deadline_minutes', '{count}분 후 마감', { count: diffMinutes });
            }
            deadlineHtml = '<span class="poll-deadline">⏰ ' + timeText + '</span>';
        } else {
            deadlineHtml = '<span class="poll-deadline expired">⏰ ' + _t('poll.deadline_expired', '마감됨') + '</span>';
        }
    }
    ...
    var closeBtn = (!isClosed && isCreator)
        ? '<button class="btn btn-sm btn-secondary" onclick="closePoll(' + poll.id + ')">' + _t('poll.close', '투표 종료') + '</button>'
        : '';
    ...
    div.innerHTML =
        ...
        '<span class="poll-total">' + _t('poll.total_votes', '총 {count}표', { count: totalVotes }) + '</span>' +
        closeBtn +
        '</div>' +
        '</div>';
...
function renderFileList(files) {
    ...
    if (!files || files.length === 0) {
        container.innerHTML = '<div class="empty-files">' + _t('files.none', '파일이 없습니다') + '</div>';
        return;
    }
    ...
    container.innerHTML = files.map(function (file) {
        ...
        return '<div class="file-item" data-file-id="' + file.id + '">' +
            ...
            '<div class="file-item-meta">' + escapeHtml(file.uploader_name || _t('common.unknown', '알 수 없음')) + ' · ' + formatDate(file.uploaded_at) + '</div>' +
            '</div>' +
            '<div class="file-item-actions">' +
            '<a href="/uploads/' + safePath + '" download class="icon-btn" title="' + _t('files.download', '다운로드') + '">⬇️</a>' +
            '<button class="icon-btn" onclick="deleteFile(' + file.id + ')" title="' + _t('files.delete', '삭제') + '">🗑️</button>' +
            '</div>' +
            '</div>';
    }).join('');
}
...
async function deleteFile(fileId) {
    if (!currentRoom) return;
    if (!confirm(_t('feature.file.delete_confirm', '이 파일을 삭제하시겠습니까?'))) return;

    try {
        var result = await api('/api/rooms/' + currentRoom.id + '/files/' + fileId, { method: 'DELETE' });
        if (result.success) {
            showToast(_t('feature.file.deleted', '파일이 삭제되었습니다.'), 'success');
            loadRoomFiles();
        } else {
            showToast(_localizedError(result, _t('feature.file.delete_failed', '파일 삭제 실패')), 'error');
        }
    } catch (e) {
        showToast(_t('feature.file.delete_failed', '파일 삭제에 실패했습니다.'), 'error');
    }
}
...
function updatePinnedBanner() {
    ...
    if (currentPins.length > 0) {
        ...
        content.textContent = latestPin.content || latestPin.message_content || _t('pin.default', '공지사항');
...
async function pinCurrentMessage(messageId, content) {
    ...
    try {
        ...
        if (result.success) {
            showToast(_t('feature.pinned', '공지로 고정되었습니다.'), 'success');
            loadPinnedMessages();
            if (typeof socket !== 'undefined' && socket && socket.connected) {
        ...
        showToast(_t('pin.create_failed', '공지 고정에 실패했습니다.'), 'error');
    }
}
...
async function deletePin(pinId) {
    if (!currentRoom) return;
    if (!confirm(_t('pin.delete_confirm', '이 공지를 삭제하시겠습니까?'))) return;

    try {
        var result = await api('/api/rooms/' + currentRoom.id + '/pins/' + pinId, { method: 'DELETE' });
        if (result.success) {
            showToast(_t('feature.unpinned', '공지가 삭제되었습니다.'), 'success');
            loadPinnedMessages();
            if (typeof socket !== 'undefined' && socket && socket.connected) {
                safeSocketEmit('pin_updated', { room_id: currentRoom.id });
            }
        } else {
            showToast(_localizedError(result, _t('pin.delete_failed', '공지 삭제 실패')), 'error');
        }
    } catch (e) {
        showToast(_t('pin.delete_failed', '공지 삭제에 실패했습니다.'), 'error');
    }
}
    ...
    try {
        ...
        container.innerHTML = roomInfo.members.map(function (m) {
            ...
            return '<div class="user-item" style="display:flex; justify-content:space-between; align-items:center; padding:12px 0;">' +
                ...
                (isAdmin ? '<span class="admin-badge">' + _t('admin.badge', '👑 관리자') + '</span>' : '') +
                '</div>' +
                '<button class="btn btn-sm ' + (isAdmin ? 'btn-danger' : 'btn-secondary') + '" ' +
                'onclick="toggleAdmin(' + m.id + ', ' + !isAdmin + ')">' +
                (isAdmin ? _t('admin.revoke_short', '해제') : _t('admin.grant_short', '지정')) + '</button>' +
                '</div>';
        }).join('');
    } catch (e) {
...
async function toggleAdmin(userId, makeAdmin) {
    try {
        ...
        if (result.success) {
            showToast(
                makeAdmin
                    ? _t('admin.granted', '관리자로 지정되었습니다.')
                    : _t('admin.revoked', '관리자 권한이 해제되었습니다.'),
                'success'
            );
            loadAdminMemberList();
            if (typeof socket !== 'undefined' && socket && socket.connected) {
        ...
        showToast(_t('admin.update_failed', '관리자 설정에 실패했습니다.'), 'error');
    }
}
...
function openChatSearch() {
    ...
    if (!chatSearch) {
        ...
        chatSearch.innerHTML =
            '<input type="text" id="chatSearchInput" placeholder="' + _t('search.in_chat_placeholder', '대화 내 검색...') + '">' +
...
function doChatSearch() {
    ...
    if (countEl) {
        countEl.textContent = chatSearchMatches.length > 0
            ...
            : _t('search.no_result_short', '결과 없음');
    }
...
async function doAdvancedSearch(loadMore) {
    ...
    if (!loadMore && !query && !dateFrom && !dateTo && !fileOnly) {
        showToast(_t('search.query_or_filter_required', '검색어를 입력하거나 필터를 선택해주세요.'), 'warning');
        return;
    }
    if (query && query.length < 2) {
        showToast(_t('search.query_min_2', '검색어는 2자 이상 입력해주세요.'), 'warning');
        return;
    }
    ...
    try {
        ...
        showToast(_t('search.failed', '검색에 실패했습니다.'), 'error');
    }
}
...
function renderAdvancedSearchResults(results, append) {
    ...
    if (!results || results.length === 0) {
        if (!append) {
            container.innerHTML = '<div class="empty-results" role="status">' + _t('feature.search.no_result', '검색 결과가 없습니다') + '</div>';
        }
        return;
    }
    ...
    var resultsHtml = results.map(function (r) {
        return '<div class="search-result-item" role="listitem" tabindex="0" onclick="goToMessage(' + r.room_id + ', ' + r.id + ')" onkeydown="if(event.key===\'Enter\')goToMessage(' + r.room_id + ', ' + r.id + ')">' +
            '<div class="search-result-sender">' + escapeHtml(r.sender_name || _t('common.unknown', '알 수 없음')) + '</div>' +
    ...
    if (append) {
        ...
        container.setAttribute('aria-label', _t('search.results_aria', '검색 결과'));
    }
    ...
    if (advSearchHasMore) {
        ...
        loadMoreBtn.textContent = _t('common.load_more', '더 보기');
        loadMoreBtn.setAttribute('aria-label', _t('search.load_more', '검색 결과 더 보기'));
...
function initOfflineBanner() {
    window.addEventListener('online', function () {
        ...
        showToast(_t('network.restored', '인터넷 연결이 복구되었습니다.'), 'success');
    });
    ...
    window.addEventListener('offline', function () {
        ...
        if (!offlineBanner) {
            ...
            offlineBanner.innerHTML =
                '<span>' + _t('network.offline', '⚠️ 오프라인 상태입니다') + '</span>' +
                '<button class="btn btn-sm" onclick="retryConnection()">' + _t('network.retry', '다시 시도') + '</button>';
...
function retryConnection() {
    if (navigator.onLine) {
        ...
        showToast(_t('network.still_offline', '아직 오프라인 상태입니다.'), 'warning');
    }
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (addPollOption) {
        addPollOption.addEventListener('click', function () {
            ...
            if (count >= 10) {
                if (typeof showToast === 'function') {
                    showToast(_t('poll.option_limit', '옵션은 최대 10개까지 가능합니다.'), 'warning');
                }
                return;
            }
            var div = document.createElement('div');
            div.className = 'poll-option-input';
            div.innerHTML = '<input type="text" placeholder="' + _t('poll.option', '옵션') + ' ' + (count + 1) + '" maxlength="100">';
